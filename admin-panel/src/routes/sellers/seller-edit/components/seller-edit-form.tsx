...
import { sellerQueryKeys, useUpdateSeller } from "@hooks/api/sellers";
...
export const SellerEditForm = ({ seller }: SellerEditFormProps) => {
  ...
  const form = useForm<z.infer<typeof SellerEditSchema>>({
    defaultValues: {
      ...
      partita_iva: getAdminSellerPartitaIva(seller) ?? "",
      rea: getAdminSellerRea(seller) ?? "",
      sdi: getAdminSellerSdi(seller) ?? "",
    },
  ...
  const handleSubmit = form.handleSubmit(async (data) => {
    const { partita_iva, rea, sdi, ...rest } = data;
    try {
      await mutateAsync({ id: seller.id, data: rest });
      await sdk.client.fetch(`/admin/sellers/${seller.id}/listing-fiscal`, {
        method: "POST",
        body: { partita_iva, rea, sdi },
      });
      await queryClient.invalidateQueries({
        queryKey: sellerQueryKeys.detail(seller.id),
      });
      toast.success(
            ...
            <div className="mt-4" data-testid="seller-edit-form-address-section">
              ...
              <div className="flex flex-col gap-y-4" data-testid="seller-edit-form-address-fields">
                ...
                <Form.Field
                  ...
                  name="partita_iva"
                  render={({ field }) => (
                    <Form.Item data-testid="seller-edit-form-piva-item">
                      <Form.Label optional data-testid="seller-edit-form-piva-label">
                        {t("sellers.fields.partita_iva")}
                      </Form.Label>
                      <Form.Control>
                        <Input {...field} data-testid="seller-edit-form-piva-input" />
                      </Form.Control>
                      <Form.ErrorMessage />
                    </Form.Item>
                  )}
                ...
                <Form.Field
                  ...
                  name="rea"
                  render={({ field }) => (
                    <Form.Item data-testid="seller-edit-form-rea-item">
                      <Form.Label optional data-testid="seller-edit-form-rea-label">
                        {t("sellers.fields.rea")}
                      </Form.Label>
                      <Form.Control>
                        <Input {...field} data-testid="seller-edit-form-rea-input" />
                      </Form.Control>
                      <Form.ErrorMessage />
                    </Form.Item>
                  )}
                ...
                <Form.Field
                  ...
                  name="sdi"
                  render={({ field }) => (
                    <Form.Item data-testid="seller-edit-form-sdi-item">
                      <Form.Label optional data-testid="seller-edit-form-sdi-label">
                        {t("sellers.fields.sdi")}
                      </Form.Label>
                      <Form.Control>
                        <Input {...field} className="font-mono" data-testid="seller-edit-form-sdi-input" />
                      </Form.Control>
                      <Form.ErrorMessage />
                    </Form.Item>
                  )}
